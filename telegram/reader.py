...
class TelegramReader:
        ...
        try:
            ...
            if not self.client.is_user_authorized():
                ...
                if not self.otp_code:
                    ...
                    return False
                self.client.sign_in(self.phone, self.otp_code)
            self.connected = True
    ...
    def get_recent_messages(self, chat, limit=5):
        ...
        out = []
        try:
            for m in self.client.iter_messages(chat, limit=limit):
                out.append({"text": getattr(m, "message", "") or "", "date": str(getattr(m, "date", ""))})
        except Exception:
            return []
        return out
